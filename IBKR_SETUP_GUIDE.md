...
1. Open the Gateway screen with the built-in **View Gateway** viewer — dashboard **Help → System Diagnostics → View Gateway**, then **⚠️ Enable keyboard / mouse control** (you need input to change this setting). No VNC client required. (The raw VNC port is not published to the host as of v5.2.108; if you specifically want an external VNC client, re-publish the port yourself — see the `ib_gateway` snippet in `docker-compose.override.yml.example` if you want an external client.)
