## [5.2.69] — 2026-07-16
### Fixed
- **The upgrade now rebuilds the View Gateway viewer — it never did, so changes to it silently didn't deploy.** `scripts/yrvi-build.sh` rebuilt `secrets`, `ib_gateway`, `scheduler`, `web` and `api`; `view-gateway` was **not in that list at all**. Because the viewer is profile-gated (compose profile `viewer`), a plain `compose build` skips it — so since the viewer shipped in v5.2.41 every source change to it has quietly failed to reach any box, while the dashboard reported the upgrade succeeded. Nobody noticed because nothing had changed in it. **v5.2.68 made it matter and, worse, made it a regression:** that release moved the viewer onto the dashboard's `/viewer/` origin, so `web` picked up the new proxy while `view-gateway` kept serving a page requesting an absolute `/websockify` — the upgrade half-landed and *broke a viewer that had been working*. Caught on the paper box by an operator noticing the viewer container hadn't restarted.
- The build now rebuilds `view-gateway` too, gated so an upgrade never starts a viewer nobody asked for: if its **image exists** (you set it up at some point) it is rebuilt; if the container is also **running** it is recreated, so the fix lands immediately; if it is **stopped**, only the image is rebuilt — `/api/view-gateway/start` runs `compose up -d`, which recreates from the newer image by itself, so it self-heals the next time you open it. Never set up? It is skipped entirely. The build keeps its per-service `timeout` guard and `failed_services` reporting, so a stuck viewer build still cannot block the rest of the upgrade.
