...
- **Existing boxes are not disturbed.** Settings saves persist the full settings object, so any box that has ever saved a setting already has `wheel_stop_loss_enabled` materialized (as `false`) and keeps its explicit choice — no surprise Monday sells from this change. Only a never-configured box inherits the new default. A running box that wants the stop loss must still turn it on in Settings.
