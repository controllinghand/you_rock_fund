...
const LABELS = {
  ...
  // discord_feedback_webhook_url is DEAD (v5.2.122): api.py ignores it and always uses the relay. Hidden — see deriveSecrets filter.
...
function deriveSecrets(statusSecrets) {
  ...
  // discord_feedback_webhook_url is hidden AND no longer read by api.py (v5.2.122): feedback
  // always goes through the You Rock Club relay. Boxes set up before v5.2.4 still hold the
  // webhook Discord deleted on 2026-09-19 in that secret, so honoring it sent them to a dead
  // URL. Kept hidden rather than shown as a stale, ignored row.
